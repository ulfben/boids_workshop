...
struct Boid final{
	...
	Vector2 calculate_acceleration(std::span<const Obstacle> obstacles){
	...
	void update(float deltaTime, Vector2 acceleration) noexcept{
		velocity += acceleration * deltaTime;
		velocity = Vector2ClampValue(
			velocity,
			globalConfig.min_speed,
			globalConfig.max_speed
		);
	...
	Vector2 obstacle_avoidance(std::span<const Obstacle> obstacles) const noexcept{
		...
		for(const auto& obs : obstacles){
			...
			Vector2 away = wrapped_offset(obs.position, position);
			float distance = Vector2Length(away);
			if(distance < safe_distance){
				steer += Vector2Normalize(away) * (safe_distance - distance);
				++count;
			}
		}
...
int main(){
	...
	std::vector<Vector2> accelerations(BOID_COUNT);
